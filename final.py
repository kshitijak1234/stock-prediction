...
def main():
    ...
    # Company selection
    symbol = st.sidebar.selectbox('Select Company', ['AAPL', 'MSFT', 'GOOGL', 'TSLA', 'FB', 'AMZN', 'BTC-USD', 'ETH-USD'])

    # Year selection using a slider
    current_year = datetime.now().year
    year = st.sidebar.slider('Select Year', min_value=2010, max_value=current_year, value=current_year)

    # Fetching stock data for the selected year
    start_date = datetime(year, 1, 1)
    end_date = datetime(year, 12, 31)
    stock_data = fetch_stock_data(symbol, start_date, end_date)
    num_days = st.sidebar.slider('Number of Days to Display', min_value=1, max_value=len(stock_data), value=30)
    ...
    recommendation, price_before, price_after, profit = generate_recommendations(predicted_price[0], current_price)
    st.sidebar.write(f"Predicted closing price for the next day: {predicted_price[0]}")
    ...
    # Data loading and analysis expander
